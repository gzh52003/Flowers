<template>
  <div>
    ...
    <div class="order-main">
      <el-row>
        <el-col :span="6">
          <el-input placeholder="请输入内容" v-model="input5" class="input-with-select order-search" @change="search" >
...
export default {
  data(){
    ...
    methods: {
     ...
     async render(){
            let res = await this.$request.get(`/order?page=${this.yema}&size=${this.num}&total=true`,)
      ...
      search(){
           if(!this.input5){
            ...
            if(t)return
             t = setTimeout(()=>{
              t = null
              this.tableData5 = this.tableData5.filter(item => item.order_number === this.input5 )
            },500)
